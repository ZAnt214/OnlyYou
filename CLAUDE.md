...
> **Referência visual complementar:** antes de redesenhar uma nova página ou fluxo importante,
> consulte também `JOBE_CHAT_DESIGN_GUIDE.md`. Ele registra os padrões de hierarquia, contraste,
> compactação, mobile, menus, CTAs e linguagem aprovados no redesenho do chat e que devem ser
> reaproveitados no restante do Jobê quando fizer sentido.
...
aprovada acima. Os exemplos antigos de laranja devem ser lidos como referências ao token de
...
| `--color-accent` / `--color-accent-hover` / `--color-accent-soft`          | Laranja Jobê — **fundos preenchidos**: CTAs, estado ativo e barrinhas decorativas |
| `--color-accent-text`                                                      | mesma laranja, em tom escuro — texto, borda, ícone e foco sobre superfície clara  |
| `--color-on-accent`                                                        | texto/ícone sobre o fundo de accent; nunca presumir branco no tema escuro           |
| `--color-highlight` / `--color-highlight-hover` / `--color-highlight-soft` | laranja fechada — ofertas, preços promocionais e pontos de energia comercial      |
...
O laranja de marca (laranja vivo no tema claro) funciona em fundos preenchidos. Para texto sobre
...
- `--color-accent-text` — em `text-*`, `border-*`, `ring-*`, `outline-*`, `accent-*` (nativo de
  checkbox/radio) quando o laranja aparece como primeiro plano sobre uma superfície clara: preços,
...
tema escuro o accent fica mais claro e o texto sobre ele passa a ser carvão profundo.
...
- **Badges/indicadores de status positivo** (aceita, aprovado, pago, verificado — exceto o
  ...
  o accent é laranja; usar os dois lado a lado em contextos parecidos
...
- **Badge de verificado**: usa o token `--color-verified` (azul), intencionalmente separado do
  `--color-accent`/`--color-accent-text` laranja — mantém a semântica de verificação (tipo
