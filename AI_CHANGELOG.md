...
- Edição do perfil, competências e idiomas agora passam por RPCs `security invoker`, com identidade
  obtida por `auth.uid()`, escopo fixo, RLS e permissões limitadas às colunas editáveis.
- A ativação usa uma RPC `security definer` isolada, necessária apenas para acrescentar o papel
  `creator` à própria conta sem liberar escrita pública em `roles`.
- `roles`, verificação, avaliações e contadores internos deixaram de ser alteráveis pela API pública.
- A tela de ativação explica os próximos passos e exige ciência dos termos e regras de segurança.
- Validação: ESLint e TypeScript concluídos; testes SQL confirmaram bloqueio de `roles`, verificação
  e avaliações; advisors revisados. O build compilou e passou pelo TypeScript, mas a geração de
  `/admin` exige as variáveis Supabase de service role, ausentes no ambiente local.

## 2026-09-22 — Refinamento editorial e visual da home

- Cabeçalho integrado ao marfim, navegação enxuta em telas intermediárias e busca com rótulo acessível.
...
- Cards de serviço com ação de 44 px e contorno discreto, sem animação de deslocamento.
- Selo de oferta neutro; preservados dados reais, rotas e operações existentes.
- Referências públicas consultadas: GetNinjas, Workana, VintePila e Packzin.
- Validação: lint e TypeScript executados; inspeção visual em navegador e build completo não realizados nesta revisão.
